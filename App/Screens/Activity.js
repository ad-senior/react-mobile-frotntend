...
import {View, ScrollView, Slider, TouchableOpacity, Alert, Modal, AsyncStorage, Platform, TouchableWithoutFeedback} from 'react-native';
...
class Activity extends Component {
    constructor (props) {
        ...
        this.state = {
            ...
            requestEmpty: false,
            moodEmpty: false,
            activity: emptyString,
            requestText: emptyString,
            moods: [],
            duration: emptyTime,
            hours: "00",
            minutes: "00",
  ...
  _validation () {
      ...
      if (this.state.duration === emptyTime) {
          isValid = false;
          durationEmpty = true;
      }
                  ...
                  this.props.submitActivity(data)
                      .then((response) => {
                          if (response.type === "POST_SUCCESS") {
                              ...
                              navigate('ActivityReview', {message: 'Activity', data, keywords, offline: true});
                          }
                      });
              }
          }).done();
              ...
              :
                  ...
                  <Modal
                      ...
                      <TouchableOpacity
                          ...
                          activeOpacity={1}
                          onPressOut={() => { this.setState({isDateTimePickerVisible: false}); }}>
                              ...
                              <View style={[styles.modal, {width: 200, maxHeight: "80%"}]}>
                                  <View >
                                      <View style={[styles.flexRow, {justifyContent: "space-evenly", alignItems: "center", marginVertical: 10}]}>
                                          {Platform.OS != platforms.ios && <View style={[styles.timePickerLine, {translateX: -18}]}></View>}
                                          {Platform.OS != platforms.ios && <View style={[styles.timePickerLine, {translateX: 82}]}></View>}
                                          <TimePicker
                                              itemSpace={80}
                                              textSize={18}
